...
if selected_company:
    ...
    # Step-by-Step Prerequisites Ingestion Pipeline Banner
    with st.container():
        st.markdown("### 🚀 Step 1: Ingest Company Filings & Corporate Actions")
        st.caption("Run all primary-source data ingestion modules in sequence to populate prerequisites for the 100-pt Research Score, Balance Sheet Forensics, and Synthesis.")

        pcol1, pcol2, pcol3, pcol4 = st.columns(4)
        with pcol1:
            step_sync_filings = st.button("1️⃣ Ingest Filings Archive", use_container_width=True, key="pipe_sync_filings")
        with pcol2:
            step_recon_fin = st.button("2️⃣ Reconstruct Statements", type="primary", use_container_width=True, key="pipe_recon_fin")
        with pcol3:
            step_auto_cat = st.button("3️⃣ Extract 8-K Catalysts", use_container_width=True, key="pipe_auto_cat")
        with pcol4:
            step_ingest_all = st.button("⚡ Run Full SEC Ingestion", type="primary", use_container_width=True, key="pipe_ingest_all")

        if step_sync_filings or research_btn:
            with st.spinner(f"Ingesting latest SEC filings for {selected_company['name']}..."):
                sec_filing_service.sync_company_filings(
                    company_id=selected_company["id"],
                    cik=selected_company.get("cik") or cik,
                    filing_repository=sec_filing_repository,
                    form_types=["10-K", "10-Q", "8-K", "DEF 14A", "13D", "13D/A", "13G", "4"],
                    limit=40,
                )
                st.toast("✅ Filings archive synchronized!", icon="📥")

        if step_recon_fin or step_ingest_all:
            with st.spinner(f"Reconstructing multi-year financial statements for {selected_company['name']}..."):
                sec_statement_reconstructor.ingest_and_save_full_history(
                    company_id=selected_company["id"],
                    cik=selected_company.get("cik") or cik,
        ...
        if step_auto_cat or step_ingest_all:
            with st.spinner("Extracting 8-K catalysts, DEF 14A proxy, 13D ownership, and Form 4 insiders..."):
                sec_catalyst_detector.extract_catalysts_from_filings(
                    company_id=selected_company["id"],
                ...
                sec_ownership_service.extract_ownership_from_filings(
                    company_id=selected_company["id"],
                    filing_repo=sec_filing_repository,
                    ownership_repo=ownership_filings_repository,
                    catalyst_repo=catalyst_repository,
                )
                sec_insider_service.extract_insider_transactions_from_filings(
                    company_id=selected_company["id"],
                    filing_repo=sec_filing_repository,
                    insider_repo=insider_transactions_repository,
                )
                st.toast("✅ Ingested corporate actions, proxy, catalysts, ownership, and insider records!", icon="⚡")

    st.divider()

    # Logically Ordered Tabs: Ingestion Prerequisites (1-6) -> Score & Valuation (7-9) -> Synthesis & Thesis (10-11)
    dossier_tabs = st.tabs([
        "📥 1. SEC Filings & Timeline",
        "📊 2. Financial Statements (5-Yr / 8-Qtr)",
        "⚡ 3. Catalysts & 8-K Events",
        "👔 4. Management & Incentives (DEF 14A)",
        "👥 5. Ownership & Activism (13D / 13G)",
        "💼 6. Insider Activity (Form 4)",
        "🏆 7. Cigar-Butt & Research Score",
    ...
    with dossier_tabs[2]:
        ...
        with cat_col2:
            extract_cat_btn = st.button("⚡ Auto-Extract 8-K Catalysts", type="primary", use_container_width=True, key="tab3_extract_btn")
    ...
    # ==========================================
    # TAB 4: MANAGEMENT & INCENTIVES (DEF 14A)
    # ==========================================
    with dossier_tabs[3]:
        ...
        with m_col2:
            ingest_mgmt_btn = st.button("⚡ Ingest DEF 14A Proxy", type="primary", use_container_width=True, key="tab4_ingest_mgmt_btn")
    ...
    # ==========================================
    # TAB 5: OWNERSHIP & ACTIVISM (13D / 13G)
    # ==========================================
    with dossier_tabs[4]:
        ...
        with own_col2:
            extract_own_btn = st.button("⚡ Extract 13D/13G Ownership", type="primary", use_container_width=True, key="tab5_extract_own_btn")
    ...
    with dossier_tabs[5]:
        ...
        with ins_col2:
            extract_ins_btn = st.button("⚡ Ingest Form 4 Transactions", type="primary", use_container_width=True, key="tab6_extract_ins_btn")
    ...
    # ==========================================
    # TAB 7: CIGAR-BUTT & RESEARCH SCORE (NOW AFTER PREREQUISITES)
    # ==========================================
    with dossier_tabs[6]:
        st.subheader("🏆 InvestiCore Research Score & Cigar-Butt Valuation")
        st.caption("Synthesizes all prerequisites: Balance Sheet Net Cash + Cash Burn + Dilution + Extracted Catalysts + DEF 14A Management.")
        ...
        if latest_p:
            ...
            # Calculations
            ncav = calculate_ncav(ca_m, tl_m) if ca_m > 0 else None
            ...
            curr_ratio = (ca_m / tl_m) if tl_m > 0 and ca_m > 0 else 0.0
            ...
            # Read Ingested Catalysts and Management from Repositories
            extracted_catalysts = catalyst_repository.list_by_company(selected_company["id"])
            has_active_catalysts = len(extracted_catalysts) > 0
            best_cat_prob = max([c.get("probability", 50) for c in extracted_catalysts], default=0.0)

            mgmt_records = management_compensation_repository.list_by_company(selected_company["id"])
            mgmt_score = None
            if mgmt_records:
                total_own = sum(float(m.get("ownership_pct", 0.0)) for m in mgmt_records)
                align_eval = sec_management_service.calculate_alignment_score(total_own, 0.75, True, True, False)
                mgmt_score = align_eval["management_alignment_score"] / 10.0  # Scale to 10 max points
            ...
            score_res = calculate_cigar_butt_score(
                ...
                catalyst_probability=best_cat_prob,
                has_active_catalyst=has_active_catalysts,
                management_score_override=mgmt_score,
            )
            ...
            with sc_c2:
                ...
                st.write(f"**Catalysts:** `{cb['catalyst']}/20` (from {len(extracted_catalysts)} events) | **Management:** `{cb['management']}/10` (from DEF 14A) | **Dilution Risk:** `{cb['dilution_risk']}/10`")

            st.divider()
            m1, m2, m3, m4 = st.columns(4)
            m1.metric("NCAV ($M)", f"${ncav:,.1f}M" if ncav is not None else "Unclassified BS")
            ...
            with st.expander("Adjust Recovery Assumptions (%) & Liquidation Wind-Down Costs", expanded=True):
                ...
                with lcol2:
                    rec_ppe = st.slider("PP&E / Land Recovery %", 0, 100, 50, step=5, key="cigar_rec_ppe") / 100.0
                    rec_gw = st.slider("Goodwill & Intangibles Recovery %", 0, 50, 0, step=5, key="cigar_rec_gw") / 100.0
                    rec_oa = st.slider("Other Assets Recovery %", 0, 100, 20, step=5, key="cigar_rec_oa") / 100.0
                with lcol3:
